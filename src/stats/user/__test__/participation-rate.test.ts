...
describe("User > participationRate", () => {
    it("특정 리뷰어가 요청 받았던 PR 중 참여한(코멘트 혹은 승인이 하나라도 있는) PR 의 비율을 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = participationRate(dc.createUser(reviewer1, {requestedPRs}));

        // then
        expect(result.value).to.be.equal((2 / 3) * 100);
    });
});
