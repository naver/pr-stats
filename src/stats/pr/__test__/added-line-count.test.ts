...
describe("PR > addedLineCount", () => {
    it("PR에서 추가된 라인 수를 구할 수 있어야 한다.", () => {
        ...
        // when
        const result = addedLineCount(dc.createPR({userId: author, additions: mockAdditions}, activities));

        // then
        expect(result.value).to.be.equal(mockAdditions);
    });
});
