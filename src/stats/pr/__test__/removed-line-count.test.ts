...
describe("PR > removedLineCount", () => {
    it("PR에서 제거된 라인 수를 구할 수 있어야 한다.", () => {
        ...
        // when
        const result = removedLineCount(dc.createPR({userId: author, deletions: mockDeletions}, activities));

        // then
        expect(result.value).to.be.equal(mockDeletions);
    });
});
