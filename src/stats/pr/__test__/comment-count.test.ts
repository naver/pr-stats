...
describe("PR > commentCount", () => {
    it("PR에서 리뷰어들이 남긴 코멘트 수를 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = commentCount(dc.createPR({userId: author}, activities));

        // then
        expect(result.value).to.be.equal(4);
    });

    it("남긴 코멘트에 내용이 있는 경우(body가 비어 있지 않은 경우)만 코멘트라고 판단한다.", () => {
        // given
        const author = "author1";
        const [reviewer1, reviewer2, reviewer3] = ["reviewer1", "reviewer2", "reviewer3"];
        const activities = [
    ...
    it("리뷰어가 승인과 함께 남긴 글도 코멘트라고 판단한다.", () => {
        // given
        const author = "author1";
        const [reviewer1, reviewer2, reviewer3] = ["reviewer1", "reviewer2", "reviewer3"];
        const activities = [
