...
import {averageRemovedLineCount} from "../";

const MOCK_CREATED_DATE = new Date("2023-09-15T14:00:00Z");
const MOCK_MERGED_DATE = new Date("2023-09-20T19:00:00Z");

describe("PRList > averageRemovedLineCount", () => {
    it("특정 PR 목록에서 PR당 제거된 라인 수의 평균을 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = averageRemovedLineCount([prStats1, prStats2, prStats3]);

        // then
        expect(result.value).to.be.equal(
            (mockChanges1.deletions + mockChanges2.deletions + mockChanges3.deletions) / 3,
        );
    });
});
