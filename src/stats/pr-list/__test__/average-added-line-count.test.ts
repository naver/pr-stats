...
import {averageAddedLineCount} from "../";

const MOCK_CREATED_DATE = new Date("2023-09-15T14:00:00Z");
const MOCK_MERGED_DATE = new Date("2023-09-20T19:00:00Z");

describe("PRList > averageAddedLineCount", () => {
    it("특정 PR 목록에서 PR당 추가된 라인 수의 평균을 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = averageAddedLineCount([prStats1, prStats2, prStats3]);

        // then
        expect(result.value).to.be.equal(
            (mockChanges1.additions + mockChanges2.additions + mockChanges3.additions) / 3,
        );
    });
});
