...
import {averageApprovalCount} from "../";

const MOCK_CREATED_DATE = new Date("2023-09-15T14:00:00Z");
const MOCK_MERGED_DATE = new Date("2023-09-20T19:00:00Z");

describe("PRList > averageApprovalCount", () => {
    it("특정 PR 목록에서 PR당 승인 수의 평균을 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = averageApprovalCount([prStats1, prStats2, prStats3]);
        const approvalCount1 = 3;
        const approvalCount2 = 2;
        const approvalCount3 = 1;

        // then
        expect(result.value).to.be.equal((approvalCount1 + approvalCount2 + approvalCount3) / 3);
    });
});
