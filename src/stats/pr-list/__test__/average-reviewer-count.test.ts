...
import {averageReviewerCount} from "../";

const MOCK_CREATED_DATE = new Date("2023-09-15T14:00:00Z");
const MOCK_MERGED_DATE = new Date("2023-09-20T19:00:00Z");

describe("PRList > averageReviewerCount", () => {
    it("특정 PR 목록에서 PR당 리뷰어 수의 평균을 구할 수 있어야 한다.", () => {
        // given
        ...
        // when
        const result = averageReviewerCount([prStats1, prStats2, prStats3]);
        const reviewerCount1 = 3;
        const reviewerCount2 = 2;
        const reviewerCount3 = 1;

        // then
        expect(result.value).to.be.equal((reviewerCount1 + reviewerCount2 + reviewerCount3) / 3);
    });
});
